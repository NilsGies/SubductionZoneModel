...
base_dir='/Users/nils/Documents/Aktuelle_Projekte/Boron_modeling/NG/calc/';
in_folder='final_nomgsur_morb';
%output_directory_path='/Users/nils/Documents/Aktuelle_Projekte/Boron_modeling/NG/final_nomgsur_morb/data_out_fsp'
% and output directory
%output_directory=[working_directory '/data_out_fsp'];
output_directory=[base_dir '/data_output'];
...
for water_sat=0 % water_sat=0 -> 2 % water in mantle ; water_sat=1 -> water saturated
    %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% selection of SZs %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for wanted_SZ = [33] %1:56   % 1) 1:8 2) 9:16 3) 17:24  4) 25:32 5) 33:40 6)  41:48 7)  49:56
        time.time_SZ(1,wanted_SZ) = datetime('now');

        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% models with the following settings %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        %
        % 1     hp02     Koma   (A-phaseK)   (mgsur)
        % 2     hp02     PW     (A-phase)    (mgsur)
        % 3     hp11     koma   (A-phAK11)   (mgsur11)
        % 4     hp11     PW     (A-phase)    (mgsur11)
        % 5     hp02     Koma   (A-phaseK)
        % 6     hp02     PW     (A-phase)
        % 7     hp11     koma   (A-phAK11)
        % 8     hp11     PW     (A-phase)
        %
        ...
        for model=1:8%[1:8]
            ...
            % now we loop through the entire PT loop
            for path_to_calculate=1:number_paths
                for j=1:10
                    %determine the size of the phases array
                    size_phase_wt = size(phasewt1{path_to_calculate,j});
                    %put the names into a matrix
                    phase_names2 = phasewt1{path_to_calculate,j};
                    %set the counter to 0
                    pos_to_write = 0;
                    %loop through the phase names
                    for k=1:size_phase_wt(1)
                        %determine the last phase entry
                        pos_empty = strmatch('empty',phase_names);
                        %check whether the names in the new matrix are already listed
                        pos_phase_name{k} = strmatch(phase_names2(k),phase_names);
                        %if the name of a phase is not listed yet the strmatch result is a
                        %0x1 matrix
                        size_pos_phase_name = size(pos_phase_name{k});
                        %if the result is a 0x1 matrix (equal to the fact that we have a
                        %new phase
                        if size_pos_phase_name(1)==0
                            %remember the position in the new matrix
                            pos_to_write = k;
                            %write the entry from the new matrix into the phase_names after
                            %the last phase entry
                            if pos_empty==0
                                phase_names(pos_empty(1)) = phase_names2(pos_to_write);
                            end
                        end
                    end
            ...
            for n=1:numel(names)
                ...
                %  all_phases.(char(names{n}))=cell2mat(phasewt_2(:,:,n));
            ...
            if write_matrices_to_file==1
                h2o_matrix_pre='free_h2o_matrix_';
                CO2_matrix_pre='free_CO2_matrix_';
                phase_matrix_pre='phase_matrix_';

                name_h2o_matrix=[h2o_matrix_pre SZ_num];
                name_CO2_matrix=[CO2_matrix_pre SZ_num];
                name_phase_matrix=[phase_matrix_pre SZ_num];
                dlmwrite(name_h2o_matrix,free_h2o_matrix);
                dlmwrite(name_CO2_matrix,free_CO2_matrix);
                dlmwrite(name_phase_matrix,phase_matrix);
            ...
            speed_m_s=SZ_info.Speedmmyr(wanted_SZ)*((0.001)/3.154e7); % m/s 3.171e-11 3.1688738506811E-11
            speed_mm_a=SZ_info.Speedmmyr(wanted_SZ);
            ...
            % xspace
            X_path_matrix_org=X_path_matrix1;

            min_matrix_X=min(min(X_path_matrix1));
            max_matrix_X=max(max(X_path_matrix1));

            X_path_matrix_inter=zeros(size(X_path_matrix1,2),size(X_path_matrix1,1));
            ...
            X_dist_matrix=repmat(X_dist_1,[size(X_path_matrix1,2),size(X_path_matrix1,1)])';
            ...
            h2o_flux_SZ_TG_Ma=h2o_flux_SZ_TG_a*1000000;
            ...
            if water_sat==0
                save(([output_dir_mat 'all_data_model_' num2str(model) '_H2O_2pct_' num2str(wanted_SZ.','%02d') '_' char(sz_names{1}(wanted_SZ))]), '-struct', 'all_data', '-v7.3')
            end

            if water_sat==1
                save(([output_dir_mat 'all_data_model_' num2str(model) '_H2O_sat_' num2str(wanted_SZ.','%02d') '_' char(sz_names{1}(wanted_SZ))]), '-struct', 'all_data', '-v7.3')
            end


            %%
            output_dir_H2O=[output_directory '/total_water_results/'];
            ...
            %% clear data
            clearvars -except wanted_SZ water_sat matrix_interpolation crust_only model total_h2o_recycled_SZ_TG_Ma sz_names working_directory output_directory time output_dir_H2O
        end %this is the end for the Model loop
        %%
%         output_dir_time=[output_directory '/time/'];
% 
%         if ~exist(output_dir_time, 'dir')
%             mkdir(output_dir_time)
%         end
%         time.time_SZ(2,wanted_SZ)= datetime('now');
%         save (([output_dir_time 'Time_SZ_' num2str(wanted_SZ.','%02d') '_' char(sz_names{1}(wanted_SZ)) '_model_' num2str(model) '_H2O_sat']), '-struct', 'time')


    end %this is the end for the SZ loop
end %this is the end for the Water settings loop
cd([base_dir, in_folder])
delete(newdir)
%% clean up
% 
% if isfile('NUL')
